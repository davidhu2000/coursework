...
def expensive_tastes
  ...
  execute(<<-SQL)
    SELECT
      styles.style, SUM(count.price) / SUM(count.track_count) as avg
    FROM
      styles
    JOIN
      (SELECT
        albums.asin, albums.price, COUNT(tracks.*) as track_count
      FROM
        albums
      JOIN
        tracks on tracks.album = albums.asin
      ...
      GROUP BY
        albums.asin) as count on styles.album = count.asin
    GROUP BY
      styles.style
    ORDER BY
      avg DESC
