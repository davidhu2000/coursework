class ShortenedUrl < ActiveRecord::Base
  ...
  def self.random_code
    ...
    code = SecureRandom.urlsafe_base64 while exists?(short_url: code)
    code
  end
  ...
  def self.submissions_last_minute
    select(:submitter_id).where('created_at > ?', 1.minute.ago)
  end

  def self.number_of_submissions_by_user(id)
    select(:submitter_id).where(submitter_id: id).count
  end
  ...
  def get_all_visits
    Visit.select(:visitor_id).where(short_url_id: id)
  end
end
